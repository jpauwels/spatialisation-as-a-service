...
function Uploader(props: UploaderProps) {
  ...
  const submitStep = () => {
    return (
      <div>
        {(file && validFile) ? <div></div> :
          <div><Typography color={'red'}>Please upload a valid file and choose the number of stems you would like it
            split into</Typography></div>}
      </div>
    );
  };
          ...
          <div style={{ display: 'flex', flexDirection: 'row' }}>
            ...
            <div style={{ flex: '1 1 auto' }} />
            {
              activeStep === steps.length - 1
                ? <Button onClick={handleUploadClick}>Finish and Upload</Button>
